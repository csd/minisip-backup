...
static string buildPluginPath(const string &argv0) {
	...
	pluginPath += prefixDir + DIR_SEPARATOR + "lib" + DIR_SEPARATOR + PACKAGE
			+ DIR_SEPARATOR + "plugins";
	return pluginPath;
}
	...
	do {
		...
		if (a && a[0] == '-') {
			...
			case 'c':
				...
				break;
			default:
				throw MinisipBadArgument(
						(string("bad argument: ") + a[1]).c_str());
				break;
			}
...
int Minisip::stop() {
	if (!sip.isNull()) { //it may not be initialized ...
		...
		SipSMCommand sipcmd(cmdstr, SipSMCommand::dialog_layer,
...
int Minisip::exit() {
	...
	Logger::getInstance()->info(string("0"), "info.MiniSIPExiting");
	...
	mout << endl << endl << BOLD << "Minisip can't wait to see you again! Bye!"
			<< PLAIN << endl << endl << endl;
	return 1;
}
...
int Minisip::startSip() {
	...
	logger->info(string("MiniSIP started"),
			"info.MiniSIPStarted");
	logger->info(logger->log_version,
			"info.loggingVersion");
	logger->info(itoa(pid), "info.pid");
	...
	try {
		...
		logger->info(localIpString, "info.myIP");

		bool done;
		int port = phoneConf->sipStackConfig->preferedLocalSipPort;
		phoneConf->logConfigurationDetails(phoneConf);
		...
		phoneConf->sipStackConfig->externalContactUdpPort
		...
		if (ip6Provider)
			phoneConf->sipStackConfig->localIp6String
					= ip6Provider->getExternalIp();
		udpSocket = NULL;
...
#ifdef DEBUG_OUTPUT
		mout << BOLD << "init 7/9: Connecting GUI to SIP logic" << PLAIN
				<< endl;
#endif
		 ...
		 */
		printf(
				"---------------------- Minisip startSip Message Router setCallBack ");
		...
		logger->info(string("Minisip caught an exception"),
				"error.MiniSIPExiting");
		merr << exc.what() << endl;
		ret = -1;
	} catch (...) {
		//FIXME: Display message in GUI
#ifdef DEBUG_OUTPUT
		merr << "Minisip caught an unknown exception (default). Quitting."
				<< endl;
#endif
		logger->info(string("-1"), "error.MiniSIPExiting");
		ret = -1;
	};
	return ret;
}
		...
		try {
			...
			if (!confBackend) {
				merr << "Minisip could not load a configuration" << endl
						...
						<< "exit." << endl;
				throw new MinisipBadArgument(
						"The configured backend could not be loaded");
				//::exit( 1 );
			}
			...
			if (retGlobal > 0) {
				...
				for (list<MRef<SipIdentity*> >::iterator i =
						phoneConf->identities.begin(); i
						!= phoneConf->identities.end(); i++) {
...
#ifdef DEBUG_OUTPUT
			merr << FG_ERROR << "Element not found: " << enf.what() << PLAIN
					<< endl;
#endif
			merr << string("ERROR: Could not parse configuration item: ")
					+ enf.what() << endl;
